"""Resolved package version and source commit for CLI help and --version."""
...
from dns_updater import _build_metadata

_PACKAGE_NAME = "cloudflare-dns-updater"
...
def _read_installed_package_version() -> str | None:
    ...
    try:
        candidate = version(_PACKAGE_NAME).strip()
...
def _resolve_commit(repo_root: Path) -> str:
    for key in ("GITHUB_SHA", "CLOUDFLARE_DNS_UPDATER_GIT_COMMIT"):
