...
@patch("cloudflare_dns_updater.cli.update_dns_entries")
@patch("cloudflare_dns_updater.cli.persist_external_addresses")
@patch("cloudflare_dns_updater.ip.discover_ipv6")
@patch("cloudflare_dns_updater.ip.discover_ipv4")
@patch("cloudflare_dns_updater.ip._read_state_file", return_value=None)
def test_run_discovers_mock_ipv4_only(
    ...
    from cloudflare_dns_updater.ip import load_external_addresses
...
@patch("cloudflare_dns_updater.ip.discover_ipv6")
@patch("cloudflare_dns_updater.ip.discover_ipv4")
@patch("cloudflare_dns_updater.ip._read_state_file")
...
def test_run_ipv6_only_change_triggers_update(
    ...
    with patch("cloudflare_dns_updater.cli.persist_external_addresses") as mock_persist:
        assert run(force=False) == 1
