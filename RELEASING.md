...
2. Release Please opens or updates a release PR (`chore(main): release X.Y.Z`) bumping
   `pyproject.toml`, `CHANGELOG.md`, and the project’s own version entry in `uv.lock`
   (via `extra-files` in `release-please-config.json`; jsonpath must use
   `@.name.value=='…'` because release-please’s TOML updater wraps string scalars).
   ...
   **Pytest (hermetic)** on that PR (bot pushes do not trigger `ci.yml`).
   CI runs `assert-uv-lock-version` **before** `uv sync` so a silent jsonpath miss
   fails the release PR instead of shipping skew.
